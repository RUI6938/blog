...
(function(window, undefined) {
				...
				if (match && (match[1] || !context)) {
					...
					if (match[1]) {
						...
						if (ret) {
							...
							selector = (ret.cacheable ? ret.fragment.cloneNode(true) : ret.fragment)
								.childNodes;
						}
	...
	(function() {
		...
		jQuery.support = {
			...
			optSelected: document.createElement("select").appendChild(document.createElement("option"))
				.selected,
			...
			if (value === undefined) {
				...
				if (elem) {
					...
					if (jQuery.nodeName(elem, "select")) {
						...
						for (var i = one ? index : 0, max = one ? index + 1 : options.length; i <
							max; i++) {
				...
				if (name in elem && notxml && !special) {
					...
					if (name === "tabIndex") {
						...
						return attributeNode && attributeNode.specified ?
							attributeNode.value :
							rfocusable.test(elem.nodeName) || rclickable.test(elem.nodeName) && elem
							.href ?
							0 :
							undefined;
					}
	...
	jQuery.event = {
		...
		fix: function(event) {
			...
			if (!event.target) {
				...
				document; // Fixes #1925 where srcElement might not be defined either
			}
			...
			if (!event.relatedTarget && event.fromElement) {
				event.relatedTarget = event.fromElement === event.target ? event.toElement : event
					.fromElement;
			}
			...
			if (!event.which && ((event.charCode || event.charCode === 0) ? event.charCode : event
				.keyCode)) {
		...
		(function() {
			...
			if (document.getElementById(id)) {
				Expr.find.ID = function(match, context, isXML) {
					if (typeof context.getElementById !== "undefined" && !isXML) {
						var m = context.getElementById(match[1]);
						return m ? m.id === match[1] || typeof m.getAttributeNode !== "undefined" && m
							.getAttributeNode("id").nodeValue === match[1] ? [m] : undefined : [];
					}
				};
	...
	jQuery.fn.extend({
		...
		domManip: function(args, table, callback) {
			...
			if (this[0]) {
				...
				if (jQuery.support.parentNode && parent && parent.nodeType === 11 && parent.childNodes
					.length === this.length) {
					results = {
						fragment: parent
					};
	...
	jQuery.extend({
		clean: function(elems, context, fragment, scripts) {
			...
			if (fragment) {
				for (var i = 0; ret[i]; i++) {
					if (scripts && jQuery.nodeName(ret[i], "script") && (!ret[i].type || ret[i].type
							.toLowerCase() === "text/javascript")) {
						scripts.push(ret[i].parentNode ? ret[i].parentNode.removeChild(ret[i]) : ret[
						i]);
	...
	jQuery.extend({
		...
		ajax: function(origSettings) {
			...
			var parts = rurl.exec(s.url),
				remote = parts && (parts[1] && parts[1] !== location.protocol || parts[2] !== location
					.host);
	...
	jQuery.fx.prototype = {
		...
		step: function(gotoEnd) {
			...
			if (gotoEnd || t >= this.options.duration + this.startTime) {
				...
				this.pos = jQuery.easing[specialEasing || defaultEasing](this.state, n, 0, 1, this.options
					.duration);
	...
	if ("getBoundingClientRect" in document.documentElement) {
		jQuery.fn.offset = function(options) {
			...
			var box = elem.getBoundingClientRect(),
				...
				top = box.top + (self.pageYOffset || jQuery.support.boxModel && docElem.scrollTop || body
					.scrollTop) - clientTop,
				left = box.left + (self.pageXOffset || jQuery.support.boxModel && docElem.scrollLeft || body
	...
	jQuery.fn.extend({
		...
		offsetParent: function() {
			return this.map(function() {
				...
				while (offsetParent && (!/^body|html$/i.test(offsetParent.nodeName) && jQuery
